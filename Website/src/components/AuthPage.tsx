...
import { User, Lock, Mail, Phone, ShieldCheck, Globe, Trophy, Award, Check } from 'lucide-react';
...
export default function AuthPage({ onLoginSuccess, onCancel }: AuthPageProps) {
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    ...
    localStorage.setItem('courtconnect_user_session', JSON.stringify(newUser));
    
    // Quick add to local users database
    const existingUsers = JSON.parse(localStorage.getItem('courtconnect_users') || '[]');
    existingUsers.push({ ...newUser, password });
    localStorage.setItem('courtconnect_users', JSON.stringify(existingUsers));
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    const matchedUser = existingUsers.find(
      (u: any) => u.email === loginEmail.trim().toLowerCase() && u.password === loginPassword.trim()
    );
    ...
    if (matchedUser) {
      const userSession: UserType = {
        ...
        membershipLevel: matchedUser.membershipLevel,
      };
      localStorage.setItem('courtconnect_user_session', JSON.stringify(userSession));
      onLoginSuccess(userSession);
    } else {
      // Default fallback demo user for standard seamless login
      if (loginEmail.trim().toLowerCase() === 'josdanielcch@gmail.com' || loginEmail.includes('@')) {
        ...
        localStorage.setItem('courtconnect_user_session', JSON.stringify(demoUser));
        onLoginSuccess(demoUser);
      } else {
        setError('Credenciales inválidas. Intente registrarse o ingrese cualquier correo para demo.');
      }
    }
  };
