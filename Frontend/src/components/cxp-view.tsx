...
import { DollarSign, Search, Filter, CreditCard, CheckCircle, Printer } from 'lucide-react'
...
export default function CxpView() {
  ...
  const handlePrint = () => {
    ...
    filteredCxp.forEach(c => {
      const name = c.supplier_name || 'Sin proveedor'
    ...
    Object.keys(groups).sort().forEach(name => {
      ...
      sections += '<div style="width:36px;height:36px;border-radius:50%;background:#fef3c7;color:#d97706;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:14px;">' + name.charAt(0).toUpperCase() + '</div>'
      ...
      sections += '<thead><tr><th style="' + thStyle + 'text-align:left;">ID</th><th style="' + thStyle + 'text-align:left;">Factura</th><th style="' + thStyle + 'text-align:left;">Fecha</th><th style="' + thStyle + 'text-align:right;">Monto Original</th><th style="' + thStyle + 'text-align:right;">Balance Pendiente</th><th style="' + thStyle + 'text-align:center;">Estado</th></tr></thead>'
      sections += '<tbody>' + rows + '</tbody></table></div>'
    })

    let html = '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Informe CxP</title><style>'
    ...
    html += '<h1>Informe de Cuentas por Pagar</h1>'
    html += '<div class="sub">Generado: ' + new Date().toLocaleString('es-DO') + ' | Filtro: ' + statusFilter + '</div>'
    html += '<div class="kg"><div class="k"><div class="kl">Deuda Total Pendiente</div><div class="kv a">$' + totalBalance.toFixed(2) + '</div></div><div class="k"><div class="kl">Total Registros</div><div class="kv">' + filteredCxp.length + '</div></div></div>'
    html += sections
    html += '</body></html>'
