...
export default function LoginView({ onLogin }: LoginViewProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Import authService dynamically or at the top of the file
      // Assuming it's default exported or named exported from @/services/authService
      const { authService } = await import('@/services/authService')
      
      if (isSignUp) {
        // Implement signup if authService has it, otherwise show error
        setError('El registro directo no está habilitado contacte a un admin.')
        return
      }

      // Use the email field as the username for login
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-4 sm:p-6 md:p-12 xl:p-16">
        <div className="w-full max-w-md xl:max-w-lg">
          {/* Card */}
          <div className="bg-card border border-border rounded-2xl shadow-2xl p-8 md:p-10 xl:p-12">
            {/* Header */}
